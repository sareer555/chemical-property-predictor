...
class ModelTrainer:
    ...
    def __init__(
        ...
        # Feature pipeline metadata: which raw-descriptor columns (in what
        # order) and what fitted scaler this model expects at inference
        # time. Set these after feature selection / scaling in the training
        # script so predict_from_raw_descriptors() can reconstruct the
        # exact input the model was trained on from a fresh descriptor
        # dict (e.g. from DescriptorGenerator.compute_all()).
    ...
    def train(
        ...
        # Force integer types for classification targets
    ...
    def _hyperparameter_search(self, X, y, param_grid=None):
        """Run GridSearchCV, subsetting large datasets for speed."""
        X = np.asarray(X)
        ...
        # Use a smaller subset for faster tuning on large datasets
        if len(X) > 5000:
        ...
        # n_jobs=1 on the base estimator avoids nested-parallelism issues
        # while GridSearchCV itself parallelizes across folds/params.
        base_model = model_class(random_state=self.random_state, n_jobs=1)
        ...
        logger.info(f"Starting GridSearchCV with param grid: {param_grid}")
    ...
    def predict_from_raw_descriptors(self, descriptors: Dict[str, float]) -> float:
        """
        Predict from a raw descriptor dict (e.g. from
        ``DescriptorGenerator.compute_all()``), reconstructing the exact
        feature subset/order/scaling this model was trained on.

        Requires ``feature_names`` (and, if the model was trained on
        scaled features, ``scaler``) to have been set - see
        ``scripts/run_full_pipeline.py`` for how they're attached before
        ``save_model()``. Falls back to using every value in
        ``descriptors``, in dict order, if ``feature_names`` isn't set
        (only correct if the model was trained directly on the full,
        unfiltered descriptor vector).

        Args:
            descriptors: Output of DescriptorGenerator.compute_all()

        Returns:
            Single prediction value
        """
        if self.feature_names:
            row = [float(descriptors.get(name, 0.0)) for name in self.feature_names]
            X = pd.DataFrame([row], columns=self.feature_names)
        else:
            row = [float(v) for v in descriptors.values() if isinstance(v, (int, float))]
            X = np.array([row])
